...
from app import app
from app.forms import LoginForm, AdministrationForm, StudentForm, GroupSelectForm, AttendanceForm
...
#global variable list for the attendance system.. should be a different mechanism, but I just wan't to get the app to work first..
...
#                conv = student_names + attendance_date
                vittu = db.session.query(Student.id, Student.full_name, Attendance.attendance).\
    join(Attendance).\
    filter(Student.id == Attendance.student_id).all()
                print(vittu)
                return vittu
        list = list()
